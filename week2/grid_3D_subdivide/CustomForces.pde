...
class NoisePotential2D extends Force {
  ...
  void apply() {
    if(on && p.isFree()) {
      float potentialX0 = noise((p.x-DELTA)*noiseScale,p.y*noiseScale);
      float potentialX1 = noise((p.x+DELTA)*noiseScale,p.y*noiseScale);
      float potentialY0 = noise(p.x*noiseScale,(p.y-DELTA)*noiseScale);
      float potentialY1 = noise(p.x*noiseScale,(p.y+DELTA)*noiseScale);
